...
export function FieldNode({ field, onDragStart }: FieldNodeProps) {
  ...
  return (
    <div
      className={`flex items-center gap-2 px-3 py-1.5 rounded transition-colors hover:bg-gray-50 ${isIgnored ? 'opacity-50' : ''}`}
      draggable
    ...
    >
      ...
      {/* Field type */}
      <span className="text-xs text-gray-400 font-mono flex-shrink-0">{field.type}</span>
      ...
      <div className="flex items-center gap-1.5 flex-wrap ml-auto">
        ...
        {isEditingLabel ? (
          <input
            ...
            className="px-2 py-0.5 text-xs border border-blue-400 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 w-32"
          ...
          <button
            onClick={handleLabelBadgeClick}
            className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-500 hover:bg-green-100 hover:text-green-700 transition-colors"
        ...
        <button
          ...
          className={`relative inline-flex h-4 w-8 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-orange-400 ${
            isIgnored ? 'bg-orange-400' : 'bg-gray-200'
          }`}
        ...
        {isIgnored && (
          <span className="text-xs text-orange-600 font-medium">ignored</span>
        )}
      </div>
    </div>
  );
