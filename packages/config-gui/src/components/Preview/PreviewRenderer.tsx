    ...
    <div className="space-y-6" data-testid="preview-renderer">
      {isLoading && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-blue-900">Updating preview...</span>
          </div>
        </div>
      )}
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <div className="border-b border-gray-200 bg-gray-50 px-4 py-3">
          <h3 className="text-sm font-semibold text-gray-900">UI Preview</h3>
    ...
    <div className="bg-white rounded border border-gray-200 overflow-hidden">
      <table className="min-w-full divide-y divide-gray-200">
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          <tr>
            {data.columns.map(col => (
              <td key={col.path} className="px-4 py-2 text-sm text-gray-500">
                {col.ignored ? (
    ...
    <div className="bg-white p-4 rounded border border-gray-200 space-y-3">
      {data.fields.map(field => (
        <div key={field.path} className="flex items-start gap-3">
          <dt className="text-sm font-medium text-gray-700 w-32 flex-shrink-0">
            {field.label}:
          </dt>
